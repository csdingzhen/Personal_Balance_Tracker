...
async function main() {
  ...
  const boa = await prisma.institution.create({
    data: {
      ...
      lastSynced: new Date('2026-05-16T08:00:00Z'),
    },
  });
  ...
  // ── Accounts ──────────────────────────────────────────────────────────────
  const boaChecking = await prisma.account.create({
    data: { id: 'acc_boa_chk', institutionId: boa.id, name: 'BOA Checking', type: 'checking', balance: 4523.18 },
  });
  const boaSavings = await prisma.account.create({
    data: { id: 'acc_boa_sav', institutionId: boa.id, name: 'BOA Savings', type: 'savings', balance: 12750.00 },
  });
  const boaCC = await prisma.account.create({
    data: { id: 'acc_boa_cc', institutionId: boa.id, name: 'BOA Cash Rewards', type: 'credit', balance: -1234.56 },
  });
  const sofiChecking = await prisma.account.create({
    data: { id: 'acc_sofi_chk', institutionId: sofi.id, name: 'SoFi Checking', type: 'checking', balance: 8901.23 },
  });
  const sofiSavings = await prisma.account.create({
    data: { id: 'acc_sofi_sav', institutionId: sofi.id, name: 'SoFi Savings', type: 'savings', balance: 25000.00 },
  });
  const sofiCC = await prisma.account.create({
    data: { id: 'acc_sofi_cc', institutionId: sofi.id, name: 'SoFi Credit Card', type: 'credit', balance: -567.89 },
  });
  const sofiInvest = await prisma.account.create({
    data: { id: 'acc_sofi_inv', institutionId: sofi.id, name: 'SoFi Invest', type: 'investment', balance: 15234.60 },
  });
  const moomooInvest = await prisma.account.create({
    data: { id: 'acc_moomoo', institutionId: moomoo.id, name: 'Moomoo Brokerage', type: 'investment', balance: 41022.04 },
  });
  ...
  for (const h of moomooHoldings) {
    await prisma.investment.create({ data: { ...h, accountId: moomooInvest.id } });
  }

  console.log('Seed complete!');
  console.log(`  3 institutions, 8 accounts, ${txData.length} transactions, ${budgets.length} budgets, ${sofiHoldings.length + moomooHoldings.length} holdings`);
}
